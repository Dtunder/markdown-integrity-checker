...
class TestMarkdownChecker(unittest.TestCase):
    ...
    def test_file_read_fallback_triggered(self):
        ...
        FailingChecker._read_file_content = fallback(
            fallback_func=MarkdownChecker._read_fallback, exceptions=(
                UnicodeDecodeError,))(
            FailingChecker._read_file_content)
    ...
    def test_file_read_retry_success(self, mock_sleep):
        ...
        RetrySuccessChecker._read_file_content = retry(
            exceptions=(
                OSError,), tries=3, delay=0.1)(
            RetrySuccessChecker._read_file_content)
